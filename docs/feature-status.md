...
| Club post/event linking | implemented | managers can now create post-only, event-only, and post-plus-event updates from the club operations rail |
...
| Personal calendar composition | implemented | `/calendar` now derives a user-specific feed from school-wide events, approved group events, timetable recurrences, and meals |
| Calendar overview/day view UX | implemented | month overview now condenses academic detail and expands into a time-based day sheet with overlap handling and resource metadata |
