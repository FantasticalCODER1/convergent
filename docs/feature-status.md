...
| Club detail hub | implemented | role-aware club detail tabs plus club-scoped operations rail for events, attendance, import guidance, certificates, and roster access |
| Club-scoped management surface | implemented | club detail now carries event editing, attendance review, import guidance, and certificate issuance instead of collapsing into the admin panel |
...
| Google Classroom references on clubs/events | partial | Classroom link, course id, post link, code, default Meet link, and resource references can be stored and surfaced with approval-aware gating, but live Classroom flows remain limited |
